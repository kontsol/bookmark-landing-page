...
const Button = ({ variant, text, className }) => {
  const baseStyles =
    "px-4 lg:px-6 py-3 rounded-sm cursor-pointer shadow-[0px_4px_4px_rgba(0,0,0,0.25)] border-2 border-transparent  ";
  ...
  return (
    <button className={`${variantStyles[variant]} ${baseStyles} ${className}`}>
      {text}
    </button>
  );
};
